...
pub fn extract_tables<'t>(
    ...
    for el in tree.select(&selector) {
        ...
        tables.push(extract_non_nested_table(
            el,
            ignored_tags,
            only_inline_tags,
        )?);
...
/// * `ignored_tags` - do not keep those tags in a cell
/// * `only_inline_tags` - if true, only inline tags are kept in a cell
pub fn extract_non_nested_table(
    table_el: ElementRef,
    ignored_tags: &HashSet<String>,
    only_inline_tags: bool,
) -> Result<Table> {
            ...
            if let Node::Element(row_el) = row_ref.value() {
                ...
                for cell_ref in row_ref.children() {
                    if let Node::Element(cell_el) = cell_ref.value() {
                        ...
                        cells.push(extract_cell(cell_ref, ignored_tags, only_inline_tags)?);
                    }
                }
...
fn extract_cell(
    ...
    Ok(Cell {
        ...
        rowspan,
        colspan,
        value: get_text_with_trace(&cell, ignored_tags, only_inline_tags),
