...
class HTMLTableExtractor:
    ...
    def __init__(self, page_url: str, html: str, parser: str = "html5lib") -> None:
        self.page_url = page_url
        self.doc = BeautifulSoup(html, parser)
    ...
    def extract_tables(
        ...
        for table_el in table_els:
            tables += self.extract(table_el, extract_context)
        ...
        if parsed_url.query == "":
            ...
            query = parsed_url.query + "&table_no="
        for i, table in enumerate(tables):
            table.id = parsed_url._replace(query=query + str(i)).geturl()
        return tables

    def extract(self, table_el: Tag, extract_context: bool = True) -> List[HTMLTable]:
        """Extract tables from html, which may contains multiple nested tables. I assume that the outer tables are used for
        ...
        tables = [HTMLTable(id="", page_url=self.page_url, **r) for r in results]
            ...
            if isinstance(c, NavigableString):
                ...
                result["elements"].append(
                    HTMLTableCellHTMLElement(
                        ...
                        attrs={str(k): v for k, v in c.attrs.items()},
